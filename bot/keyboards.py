...
# --- MAX --------------------------------------------------------------------------
# ОСНОВНОЕ — inline-кнопка на каждом ответе (`max_klaviatura`). Команду в меню бота
# пробовали (заказчик просил «команду вместо кнопки над каждым сообщением»), но
# в клиенте MAX меню НЕ ПОКАЗЫВАЕТСЯ: проверено вживую 2026-07-22 и с кириллическим
# именем «связь», и с латинским «svyaz» — `PATCH /me` отвечает 200, `GET /me` команду
# отдаёт, а слева от поля ввода её нет. Команды всё равно продолжаем выставлять
# (дёшево, best-effort) — если клиент научится их показывать, всё уже готово.
...
def max_nuzhna_svyaz(text: str) -> bool:
    """Текст в MAX — это вызов связи словами или командой (запасной путь к кнопке)?"""
...
def max_klaviatura() -> list[dict]:
    ...
    Постоянной клавиатуры под полем ввода в MAX нет, поэтому кнопка вешается на
    КАЖДЫЙ ответ бота: так она всегда под последней репликой. Нажатие приходит
    апдейтом `message_callback` (обязателен в подписке `types`).
    """
    return [{
